...
export function renderMovieList(
  ...
  // Update stats
  // Liczenie obejrzanych i nieobejrzanych z FILTROWANEJ listy
  const watchedCounts = filteredMovies.filter((m) => m.watched).length;
  const unwatchedCount = filteredMovies.length - watchedCounts;

  const totalMovies = movies.length; // <-- wszystkie z bazy, bez filtrowania!

  updateStats(watchedCounts, unwatchedCount, lang, totalMovies);
}
